...
class OpenChatConsumer(AsyncWebsocketConsumer):
    """公開ルーム用。ログイン済みなら誰でも参加・発言可能。"""

    async def connect(self):
        self.room_id = self.scope['url_route']['kwargs']['room_id']
        self.room_group_name = f'open_room_{self.room_id}'
        user = self.scope.get('user')
        if not user or not user.is_authenticated:
            await self.close()
            return
    ...
    def room_exists(self):
        from .models import open_room
        return open_room.objects.filter(pk=self.room_id).exists()
    ...
    @database_sync_to_async
    def save_message(self, content):
        from .models import open_room, open_room_message
        try:
            room = open_room.objects.get(pk=self.room_id)
        except open_room.DoesNotExist:
            return None
        return open_room_message.objects.create(
            room=room, sender=self.scope['user'], content=content
        )